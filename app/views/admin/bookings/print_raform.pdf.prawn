 	...
 	pdf.text "Equipment to be used: ", :align => :left, :size => 10, :style => :italic, :color => "#AAAAAA"
  	...
  	pdf.text @raform.equipment, :align => :right, :color => "FF0000"
         ...
         pdf.grid([2,0], [3,5]).bounding_box do
          ...
          if @raform.complex
            pdf.stroke do
             pdf.rectangle [120,122], 10, 10
             pdf.fill_circle [125, 117], 2
             pdf.fill_color "333333"
            end
          else
            pdf.stroke do
     ...
     pdf.grid([4,0], [5,5]).bounding_box do
      ...
      pdf.grid([5,0], [6,2]).bounding_box do
       ...
       pdf.grid([i,0], [i+1,2]).bounding_box do
          # color => black
          pdf.fill_color "333333"
        pdf.text hazard.name, :align => :center, :size => 10, :style => :bold, :color => "#AAAAAA"
       end
       
        pdf.grid([i,2], [i+1,4]).bounding_box do
            # color => black
            pdf.fill_color "333333"
        pdf.text hazard.persons, :align => :center, :size => 10, :style => :bold, :color => "#AAAAAA"
        end

        pdf.grid([i,4], [i+1,6]).bounding_box do
            # color => black
            pdf.fill_color "333333"
        pdf.text hazard.level, :align => :center, :size => 10, :style => :bold, :color => "#AAAAAA"
        end

        pdf.grid([i,6], [i+1,8]).bounding_box do
            # color => black
            pdf.fill_color "333333"
        pdf.text hazard.rassociation, :align => :center, :size => 10, :style => :bold, :color => "#AAAAAA"
        end

        pdf.grid([i,8], [i+1,11]).bounding_box do
            # color => black
            pdf.fill_color "333333"
        pdf.text hazard.controls, :align => :center, :size => 10, :style => :bold, :color => "#AAAAAA"
        end
